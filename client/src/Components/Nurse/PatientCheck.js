...
export default function PatientCheck() {
   ...
   const checkupdate = (e)=>{
    ...
    if (Object.keys(formErrors).length === 0 && isSubmit) {
        ...
        axios.post("http://localhost:4000/patient/view-checkup-update",formData)
            .then((response) => {
                console.log(response);
                const details = response.data.details;
                setCheckupData(details)
                setNewopen(true);
                setOpen(false);
                ...
                <div className='container'>
                    <div className='row'>
                        <div className='col-lg-12 col-md-12 col-12'>
                            ...
                            <Button onClick={submit} sx={{ marginLeft: 50, marginTop: 2, backgroundColor: 'green' }} variant="contained" data-toggle="modal" data-target="#exampleModalLong">View Patient</Button>
                            <Modal
                                open={open}
                                onClose={handleClose}
                            ...
                            <Box sx={{ marginLeft: 50, marginTop: 3 }}>
                                <TextField onChange={(e)=>{setFormData({ ...formData, patientID: e.target.value })}} label="Enter Patient ID" name='patientID' variant="outlined" />
                            </Box>
                            <Button onClick={checkupdate} sx={{ marginLeft: 50, marginTop: 2, backgroundColor: 'green' }} variant="contained" data-toggle="modal" data-target="#exampleModalLong">Check-Up & Update</Button>
